...
export type LinkCardProps = {
   ...
   *
   * ⚠️ **슬롯은 자신이 카드의 직계 자식임을 전제로 쓴다.** 폼은 마운트할 때 "이 폼을 연 연필이
   * 내 카드 안에 있는가"로 포커스를 데려갈지 정하는데(J3 의 `owned` 판정), 그 기준 상자가
   * `parentElement` — 곧 이 카드다. 슬롯을 감싸는 요소를 하나라도 끼우면 그 판정이 감싼 상자를
   * 보게 되어, 같은 링크가 두 자리에 놓였을 때 누르지도 않은 카드가 포커스를 가져간다.
   *
   ...
   * J3 착수 전에 반드시 읽어라.
   *
   * ⚠️ **오버레이도 자신이 이 카드의 직계 자식임을 전제로 쓴다** — `editSlot` 과 같은 이유다.
   * "누른 카드의 확인창인가"(`owned`)를 `parentElement.contains(트리거)` 로 가리므로, 사이에
   * 감싸는 요소를 끼우면 기준 상자가 바뀌어 판정이 어긋난다.
   *
   * ⚠️ 두 슬롯의 마운트 포커스는 **`focus({ preventScroll: true })`** 다. 값을 치르는 쪽은
   * "화면 밖 카드의 폼으로 스크롤이 따라가지 않는다" 이지만, 빼면 더 나쁘다 — 트리거를 붙들지
   * 못한 orphan 경로(클릭이 버튼에 포커스를 주지 않는 환경)에서는 가리지 않고 데려오므로,
   * 목록을 훑던 사람의 화면이 엉뚱한 카드로 점프한다. 지우지 마라.
   *
