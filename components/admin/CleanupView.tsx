 ...
 *
 * ③ 목록의 **쪽 넘김은 반대로 URL 이 아니다** — 이미 받아 둔 배열을 자를 뿐 서버에 물을 것이 없어
 * 통계 쪽 선례를 따른다(CleanupLinkList "쪽 넘김"). 그래서 아래 부제가 세는 `abandoned.length` 는
 * 지금 쪽이 아니라 **총 건수**이고, 그것이 맞다: 기본 기준에서 211건이 쌓였다는 사실이 맥락이다.
 *
      ...
      <section aria-label="오래 손대지 않은 링크" className={PANEL}>
        ...
        {abandoned === null ? (
          ...
          /* 프로토타입 587행이 장식으로 두었던 체크 자리가 여기서 실제 체크가 된다 — 방치 목록은
             한 쪽에 그린 줄이 전부 보이므로 구역 전체 선택도 함께 둔다(CleanupLinkList 주석).

             `key` 는 **기준 탭 전환에서 쪽 번호를 1로 되돌리기 위한 것이다.** 목록은 20개씩 끊어
             그리는데(CleanupLinkList 의 쪽 넘김) 기준일이 바뀌면 rpc 를 다시 쳐 목록이 통째로
             갈린다 — 다른 기준의 목록은 다른 목록이라 앞 기준의 8쪽을 이어받을 자리가 없고, 남은
             체크도 이 화면이 금하는 "보이지 않는 선택"이 된다. 목록 안에서는 이 두 경우(삭제로
             짧아진 것 / 기준이 바뀐 것)를 `items` 만 보고 가릴 수 없으므로 — 365일 결과는 180일
             결과의 부분집합이라 '줄어들었다'가 같은 모양이다 — 바깥에서 알려 준다.
             지우면 CleanupView.test 의 "기준 탭이 바뀌면 1쪽부터" 가 먼저 깨진다. */
          <CleanupLinkList
            key={retentionDays}
            label="오래 손대지 않은 링크"
            items={abandoned.map(toItem)}
          />
        )}
      </section>
    </div>
