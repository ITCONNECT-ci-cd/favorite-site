...
/**
 * `onSearchClick`·`onAiClick` 은 G5 에서 required 가 됐다 — 콜백 없는 헤더는 제품에 없다.
 * 생김새만 보는 테스트가 매번 noop 두 개를 적지 않도록 여기서 기본값을 채운다.
 * (배선 그 자체는 PaletteHost.test.tsx 가 본다. 여기는 헤더가 무엇을 부르는지까지다.)
 */
...
describe('Header', () => {
  it('검색창은 입력이 아니라 버튼형 트리거다 (실제 입력은 팔레트가 받는다)', () => {
    renderHeader();
  ...
  /*
   * C4 에는 "콜백을 주지 않아도 클릭이 터지지 않는다"가 있었다. G5 에서 두 콜백이 required 가
   * 되면서 그 상황 자체가 타입에서 막히므로 테스트를 지우고, 그 자리를 열림 상태 스레딩이 받는다.
   */
  it('isSearchOpen 을 주지 않으면 aria-expanded 는 false 다 (닫힘도 상태다)', () => {
    renderHeader();

    // 속성을 통째로 빼면 보조기술이 "펼칠 수 있는 것"임을 알 길이 없다 — 닫힘은 false 로 알린다.
    expect(searchTrigger()).toHaveAttribute('aria-expanded', 'false');
  });

  it('isSearchOpen 이면 aria-expanded 가 true 다 (열림 상태의 소유자는 G5 PaletteHost)', () => {
    renderHeader({ isSearchOpen: true });
  ...
  it('카운트는 넘겨받은 값을 그대로 쓴다 (0건도 같은 형식)', () => {
    renderHeader({ totalCount: 0, faviconCount: 0 });
