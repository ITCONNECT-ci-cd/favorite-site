...
4. Read `favorite_list_categories` once immediately before processing the message. A leaf is a category whose ID is not used as another category's `parentId`. Pick the best leaf from message context and hostname; when those are insufficient for a reliable classification, choose the leaf named `분류 대기`.
