...
6. For each URL, call `favorite_ingest` once in appearance order. Because URL fetching is disabled, set `title` to the URL hostname and `description` to an empty string. Never invent tags.
