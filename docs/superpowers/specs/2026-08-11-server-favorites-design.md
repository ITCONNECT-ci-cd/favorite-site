...
### 4.1 데이터 — 마이그레이션 `0013_server_favorites.sql`

> 원 설계 시점에는 `0008_server_favorites.sql`로 계획했지만, Discord 자동 수집 마이그레이션
> `0008`~`0012`가 먼저 운영에 적용되었습니다. 통합 이력과 원격 ledger를 보존하기 위해 같은
> 멱등 DDL을 `0013_server_favorites.sql`로 적용합니다.
