...
- [x] **I4. 링크 표 + 인라인 편집 + 고정** `L` — 의존: I1, I2, I3 ✅ 2026-08-10 새벽 완료 (fd67a06 + I5 편입 픽스업 — 스펙 조건부 승인·통합 품질 리뷰 종결 판정. Minor 잔여는 M-6 처리 완료·나머지 기록)
  - **스펙 리뷰 조건부 승인**(위반 1: PIN cursor-pointer 누락·수치 나머지 전건 일치, 판단 10건 중 9 승인·⑧ 조건부 — 설명 입력 readOnly 권고). 픽스업 5건은 impl-I5에 편입(같은 파일 편집 중 — 충돌 회피). 스윕 추가: 클릭 수 칸 접근 이름(LinkCard 공통 교차 결정), pendingResult 헬퍼 공용화+매달린 프라미스 4곳(CategoryPanel.test:436·282, CategoryHeader.test:194·295), mutations deleteCategory 복구 절차 ② 문서 정정(I4 표는 선택 상위의 하위만 나열 — 실행 불가). I5에 요구: 필터 상태 드롭→전체 id 전송 테스트. ①의 귀결(헤더 flex-wrap 없음 — 좁은 폭 라벨 이탈)은 <820px 수동 확인 항목.
  - 구현 완료(fd67a06, 테스트 52·돌연변이 22/23 RED·통합: 13번째 고정 실거부·/daily 반영·118행 재부여→복원·**290행 전 필드 대조 0건**·프로덕션 무누출 6/6). AdminLink 8칸(sort_order 제외 — 배열 자리가 순서), 하위 select 왕복 복귀 실결함을 useOptimistic+isPending으로 수정. **오케스트레이터 결정(미결 판단): 비직접 정렬 모드에서 드롭 무시+손잡이 비활성(impl-I5 반영).** 스윕 추가: CategoryPanel.test.tsx:423 매달린 프라미스 함정. 참고: Tailwind v4는 flex-[1_1_240px]를 flex:240px로 축약(동치 — 비대칭 값 쓸 때 생성 CSS 확인). 자기 신고: 비밀번호 콘솔 노출 1회(파일·커밋 무관 — 누적 4건, 아침 교체 권고).
...
- [x] **I5. 필터·정렬** `S` — 의존: I4 ✅ 2026-08-10 새벽 완료 (83579ce+b730c14+f742345 + fixup cd7e2fa — 스펙·품질 리뷰 전 항목 반영: 사라진 하위 눌림 접기, 정렬은 카테고리를 넘어 유지(⑨ 확정), 동점 픽스처·ㅎ계열 픽스처로 갈림 전부 잠금)
  - **통합 품질 리뷰: I4 종결 가능(Minor만 — M-6 한 줄 처리 권장), I5 픽스업 3건 후 종결** — I-1(없어진 하위 subFilter를 표시 단계에서 접기 — 유일한 실결함: 아무 칩도 안 눌린 채 걸러진 표), I-2(밀려난 draggingId JSDoc 이동), I-3(clicks·name 동점 픽스처 — 안정 정렬 계약 무커버). 권장: M-1·M-2 주석 정정, M-3 상수 위치, M-6(하위 select 성공 후 거동 단언). 트랙 공통 기록: M-5(dragOver preventDefault 무단언 — I1부터 공통), M-4(행 grab 커서 근거), M-7(Row 265행 분해 후보), 파일 간 중복(visibleLinks·moveOnto·cssUrl — lib 승격 열릴 때 일괄). 스윕 합류 항목 없음.
  - **스펙 리뷰 조건부 합격**(위반 0·수치 18/19 일치+갈림 ① 승인·그리기/보내기 계약 실증·I4 편입 5건 전건 이행). 판단 ⑨ **오케스트레이터 확정: 정렬은 카테고리 전환 시 유지**(검색·칩만 리셋 — 프로토타입 관례, ⑦ 문구가 혼란 커버). fixup-I5 진행(품질 I-1~I-3 + 스펙 Minor 2 + ⑨ + M-1·2·3·6).
  - 구현 완료(83579ce+b730c14+f742345, 테스트 +46·칩 숫자 DB 전건 일치·290행 전 필드 대조 무변경). FilterRow 소유 문맥, 그리기/보내기 분리 실증(숨은 id 포함 전송 테스트), 비직접 정렬 드롭 무시(오케스트레이터 결정), I4 픽스업 5건 편입. 유일 동작 갈림: 카테고리 전환 시 검색·정렬까지 리셋(프로토타입은 칩만 — 근거 기록, 게이트에서 뒤집기 쉬움). O3 노트: 칩 12개 두 줄 감김·칩 클릭 반영 실브라우저 확인(아침), ICU ko 정렬 환경 의존, 칩 숫자는 거르기 전 기준.
...
## 7-3. O3 게이트 결과 (2026-08-10 새벽, 보안 검수)

**코드 게이트 통과** — 6/6 항목 통과 (보고서: docs/superpowers/gates/O3-stage3-report.md). ①공개 화면 관리 UI 0건·②미인증 관리 URL/액션 거부는 프로덕션 빌드 미인증 실측(카드 118장 페이지 포함 연필·휴지통·칩·/admin 0), ③실패 사유 비구분(타입 봉인)·④13번째 고정 차단(트리거)·⑤삭제 확인·⑥<820px 1단(코드+테스트)은 코드 근거. C-1(cookies() 예외 삼킴) 무영향 재확인(라우트 전부 ƒ 유지), C-2(AI 자리 무동작) DEFER 정상. 전체 1461 통과.

**⚠️ 시스템 보안 전제 미완 — 아침 필수 2건 완료 전까지 REST 직접 쓰기 열림**: ⑦대시보드 signup 끄기 + ⑧0002 마이그레이션 실행. anon 키 공개+signup 켜짐 → 누구나 인증 사용자화 → 0002 미적용 RLS가 전체 쓰기 허용 = 앱 게이트 우회 경로. **코드 게이트 통과 ≠ 시스템 보안 완결**. 완료 후 `npx tsx scripts/verify-schema.ts` 8종 통과로 확인. 아침 실기기 확인 6건(카드 부활 프레임·즐겨찾기 반영·IME Enter·칩 감김·<820px·오버레이 포커스)은 보고서 참조.

## 7-4. O4 게이트 결과 (2026-08-10 새벽)

**코드 게이트 통과** — 6/6 (보고서: docs/superpowers/gates/O4-stage4-report.md). ①순위 unique·②gap 규칙·③rate limit/bulk 118·⑥<820px는 코드+테스트 통과, ④방치 경계·⑤고정 제외·①③ SQL 부분은 **라이브 DDL 미적용이라 DEFER**(아침 rpc 스팟체크). 1559 통과·미인증 누출 0·SQL 삼중 방어 정합. **아침 필수 확장: O3의 signup+0002 → 마이그레이션 3종(0002·0003·0004) 전체 적용 + signup 차단 + verify-schema 10종 PASS + 관리자 rpc 스팟체크(KST 경계·익명 42501·방치 경계·고정 제외).** 미적용 상태 우아한 실패 확인(통계 안내 카드, 정리 ①②만+③ 안내).

## 7-5. O5 최종 게이트 결과 (2026-08-10)

**5단계 코드 게이트 통과 5/5 · 프로젝트 전체 코드 완주 확정** (보고서: docs/superpowers/gates/O5-stage5-report.md). ①질의 10개(smoke+N2 라이브 10/10)·②근거+tookMs·③타자 미호출/0건↵·④폴백 5경로·⑤확장성 580건 전부 PASS. 최종 스위트 1646·tsc·eslint 청정. O1~O5 전 게이트 코드 통과. 게이트 발견: .env.example에 AI_SEARCH_API_KEY 누락 → 추가 완료. **하드 블로커(유일): 아침 사용자 작업 — 마이그레이션 3종+signup+verify 10종+rpc 완료 전까지 배포 불가(REST 직접 쓰기 우회 경로). 배포 승인 단일 관문 = verify-schema 10종 PASS.**

## 7-6. 배포 후 대응 (2026-08-10, 운영 시작 이후)

라이브 운영을 시작하고 사용자가 실제로 쓰면서 나온 것들. 전부 구현→리뷰→(필요시)픽스업을 거쳤다.

| 건 | 원인·처분 | 커밋 |
|---|---|---|
| 배포 실패 | Vercel 이 프로젝트를 정적 사이트로 감지해 빌드 후 `public/` 을 찾음 → `vercel.json` 에 `framework: nextjs` 명시 | `bbf30fa` |
| 관리자 로그인 불가 | 대시보드에서 비밀번호를 바꾸면 `.admin-credentials.local` 이 낡은 값이 된다(설계상 자동 갱신 없음). 대시보드·복구 이메일 경로가 막혀 service role Admin API 로 재설정 + 로그인 검증, 파일 갱신 | (임시 스크립트, 미커밋) |
| "통계가 0" | 코드 정상. 기록된 28건이 전부 bulk 였고 **통계는 bulk 를 의도적으로 제외**(G4 규칙). 개별 클릭 적재는 프로덕션 실측으로 확인. 유령 클릭 28건 삭제 | — |
| 홈에서 링크 추가 불가 | 카드 그리드 첫 칸에 `+ 링크 추가` 타일 → 그 자리에서 인라인 폼(분류 선택 포함). **파생 목록(즐겨찾기·매일)에는 두지 않는다** — 거기서 '추가'는 의미가 어긋난다 | `75bcc5e` |
| 정리 도구 가독성·조치 부재 | 도메인 그룹 기본 접힘 + 세 구역 체크박스 다중선택·일괄삭제(확인 1회). 리뷰가 잡은 확인 단계 구멍 2건(문구 미낭독·더블클릭이 삭제에 떨어짐) 픽스업 | `cd055f6`·`6e1778d` |
| "한 번에 열기" 무동작 | **브라우저 팝업 차단.** `noopener` 로 열면 규격상 항상 null 이라 감지 불가 → 조용히 실패하고 열리지도 않은 링크가 기록됐다. 핸들을 받는 형태로 바꾸되 `opener` 즉시 차단, **열린 것만 기록**, 차단 시 실행 가능한 안내 | `664a901` |
| 로딩 1.4초 | SQL 문제 아님(이미 3병렬). **함수가 iad1(미 동부), Supabase 는 서울** → 쿼리마다 태평양 왕복. 함수를 `icn1` 로 이전 + Cloudflare 를 DNS-only 로(사용자) → **TTFB 1.02~1.56s → 0.21s** | `8e0ff29` |
| 매일 고정 12개 상한 | 실사용이 20개 안팎으로 늘어 상한이 사용을 막음 → **상한 자체 해제**(트리거 제거). verify 검사 ④ 방향 반전("13번째가 통과하는가") | `0895a03` + `0005_lift_pin_limit.sql` |
| 테스트 1건 적색 | Windows `core.autocrlf=true` 가 작업 트리를 CRLF 로 만들어 `split('\n')[0]` 단언이 깨짐(커밋 blob 은 LF) → `split(/\r?\n/)` | `c38dfc8` |

**최종 상태**: 마이그레이션 0001~0005 전부 적용, `verify-schema` **10종 전부 PASS**, 테스트 1771 통과·tsc·eslint 청정, 프로덕션 warm TTFB 0.21s(`icn1::icn1`).

**남은 백로그(비차단)**: 팔레트 데이터 지연 로딩(홈 HTML 222KB 감축), 정리 도구가 "줄이 너무 많다"는 원 불만을 실제로 해소했는지 사용자 확인, §6-1 및 각 게이트 보고서의 기존 항목들.
...
- 2026-08-10: **5단계 완전 종료 + 프로젝트 전체 코드 완주.** N1(Gemini 방식 결정)·N2(Gemini 백엔드·라이브 10/10)·N3(팔레트 AI 영역) 종결. **O5 최종 게이트 5/5 통과** — O1~O5 전 게이트 코드 통과, 최종 1646 테스트 green. 전 스토리(EPIC A~N)가 구현→스펙 리뷰→품질 리뷰→픽스업→종결 완료. 하드 블로커: 아침 사용자의 라이브 마이그레이션·설정(코드로 불가). 아침 종합 리포트로 인계.
- 2026-08-10 새벽: **4단계 완전 종료.** K1(집계 SQL 0003)·K2(통계 화면)·L1(rate limit)·M1(정리 판정 0004)·M2(정리 화면) 전부 구현→리뷰→픽스업→종결. verify-schema 10종 체계(0002·0003·0004 적용 감지). N1(AI 방식) = Gemini LLM 직접 호출 확정. **O4 코드 게이트 통과** — SQL 부분은 아침 마이그레이션 후 확정. 5단계(N2 Gemini 백엔드) 착수.
- 2026-08-10 새벽: **3단계 완전 종료.** 스토리 12개(H1~H4·I1~I5·J1~J3) 전부 구현→스펙 리뷰→품질 리뷰→픽스업→재검증. O3 전 정리 스윕 A~O 15항목(REQUEST_FAILED 정본화·rendersSomething/focus/pending 헬퍼 추출·eslint supabase/admin 전역 반전·낱말 통일 등, 커밋 10개). **O3 게이트 통과(코드)** — 단 시스템 보안은 아침 2건 전까지 미완. 4단계(K·L·M) 착수.
