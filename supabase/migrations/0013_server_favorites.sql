-- 0013_server_favorites.sql — 즐겨찾기를 브라우저에서 DB 로 옮긴다
--
-- 적용 방법: Supabase CLI migration 으로 적용한다. 재실행 안전(`if not exists`).
--
-- ⓘ 통합 이력 (2026-08-22)
--   서버 즐겨찾기는 원래 main 브랜치의 `0008_server_favorites.sql` 로 구현돼 라이브 스키마에도
--   이미 적용됐다. 그 사이 Discord 브랜치가 `0008_refresh_operating_sites.sql` 부터 `0012` 까지를
--   원격 migration history 에 등록했다. 두 `0008` 을 함께 둘 수 없으므로, 기존 원격 이력을
--   보존하면서 이 재실행 안전 DDL 을 다음 번호 `0013` 으로 옮긴다.
--
-- ## 왜 옮기나
...
-- 인덱스를 만들지 않는다 — 수백 행이고 getAllData 가 전 행을 읽어 JS 에서 거른다.
